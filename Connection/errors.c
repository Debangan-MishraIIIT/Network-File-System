...
void handleNetworkErrors(char *error)
{
    if (!strcmp(error, "socket"))
        printf(MAG "ERROR 901: UNABLE TO OPEN SOCKET (socket)\n" reset);
    
    if (!strcmp(error, "bind"))
        printf(MAG "ERROR 902: UNABLE TO BIND TO SOCKET (bind)\n" reset);

    if (!strcmp(error, "listen"))
        printf(MAG "ERROR 903: UNABLE TO LISTEN TO INCOMING CONNECTIONS (listen)\n" reset);

    if (!strcmp(error, "accept"))
        printf(MAG "ERROR 904: UNABLE TO ACCEPT CONNECTION (accept)\n" reset);
    
    if (!strcmp(error, "connect"))
        printf(MAG "ERROR 905: UNABLE TO CONNECT TO SERVER (connect)\n" reset);
    
    if (!strcmp(error, "recv"))
        printf(MAG "ERROR 906: FAILED TO RECIEVE FROM SOCKET (recv)\n" reset);
    
    if (!strcmp(error, "send"))
        printf(MAG "ERROR 907: FAILED TO SEND TO SOCKET (send)\n" reset);
}